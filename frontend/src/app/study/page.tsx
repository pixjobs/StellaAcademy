      ...
      <div className="max-w-8xl mx-auto space-y-6">
        ...
        <div className="lg:hidden relative z-30">
          <button
            ...
            className="flex items-center justify-between w-full px-4 py-3 rounded-2xl bg-slate-900/90 border border-white/15 text-sm font-semibold hover:bg-slate-800 transition-all shadow-lg backdrop-blur-md"
          >
            <div className="flex items-center gap-2.5 min-w-0">
              <span className={`w-2.5 h-2.5 rounded-full ${activeAccent.dot} animate-pulse shrink-0`} />
              <div className="flex flex-col text-left truncate">
                <span className="text-xs text-slate-200 font-bold truncate">{activeModule.title}</span>
                <span className="text-[10px] text-slate-400 font-mono uppercase tracking-wider">
                  {activeModule.category} · LVL {activeModule.difficulty}
                </span>
              </div>
            </div>
            <span className="text-xs text-indigo-300 font-mono font-semibold flex items-center gap-1.5 shrink-0 bg-indigo-500/10 px-3 py-1.5 rounded-xl border border-indigo-500/20">
              Browse ({filteredModules.length})
              <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-300 ${mobileSelectorOpen ? 'rotate-180' : ''}`} />
            </span>
          </button>
          ...
          {mobileSelectorOpen && (
            <>
              ...
              {/* Drawer Container */}
              <div className="absolute top-full left-0 right-0 mt-2 p-4 rounded-2xl bg-slate-950/95 border border-white/15 shadow-2xl space-y-4 max-h-[80vh] overflow-y-auto backdrop-blur-2xl z-50 animate-in fade-in slide-in-from-top-3 duration-200">
                {/* Header & Close */}
                <div className="flex items-center justify-between border-b border-white/10 pb-3">
                  <div className="flex items-center gap-2">
                    <Filter className="w-4 h-4 text-indigo-400" />
                    <span className="text-xs font-bold text-slate-200 uppercase tracking-widest font-mono">Select Study Module</span>
                  </div>
                  <button
                    onClick={() => setMobileSelectorOpen(false)}
                    className="p-1 rounded-lg text-slate-400 hover:text-white bg-white/5 hover:bg-white/10 text-xs font-mono px-2 py-1"
                  >
                    Close ✕
                  </button>
                </div>
                ...
                {/* Sticky Search Input */}
                <div className="sticky top-0 z-10 pt-1 pb-1 bg-slate-950">
                  <div className="relative group">
                    ...
                    <input
                      type="text"
                      placeholder="Search by title, topic, or discipline..."
                      value={searchQuery}
                      onChange={(e) => setSearchQuery(e.target.value)}
                      className="w-full bg-slate-900 border border-white/15 rounded-xl py-2.5 pl-9 pr-9 text-xs text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 shadow-inner"
                    />
                    {searchQuery && (
                      <button
                        onClick={() => setSearchQuery('')}
                        className="absolute inset-y-0 right-0 pr-3 flex items-center text-sm font-bold text-slate-400 hover:text-white"
                      >
                        ×
                      </button>
                    )}
                ...
                <div className="space-y-1.5">
                  <span className="text-[10px] font-mono text-slate-400 uppercase tracking-wider block px-1">Discipline Filters</span>
                  <div className="flex gap-1.5 overflow-x-auto pb-1.5 scrollbar-hide">
                    {categories.map(category => (
                      <button
                        key={category}
                        onClick={() => setActiveCategory(category)}
                        className={`capitalize whitespace-nowrap px-3 py-1.5 rounded-xl text-[11px] font-semibold border transition-all ${
                          activeCategory === category 
                            ? 'bg-indigo-500/25 text-indigo-200 border-indigo-500/40 shadow-sm' 
                ...
                <div className="text-[10px] font-mono text-slate-400 px-1 flex items-center justify-between">
                  <span>Showing {filteredModules.length} of {studyModules.length} modules</span>
                  {activeCategory !== 'all' && (
                    <button 
                ...
                {/* Modules grid list */}
                <div className="grid sm:grid-cols-2 gap-2.5 pb-2">
                  {filteredModules.length > 0 ? (
                    filteredModules.map(module => {
                      ...
                      return (
                        <button
                          ...
                          className={`text-left p-3.5 rounded-xl border transition-all min-h-[64px] flex flex-col justify-between ${
                            isSelected
                              ? 'bg-slate-900 border-indigo-500/60 shadow-[0_0_16px_rgba(99,102,241,0.2)]'
                              : 'bg-slate-900/50 border-white/5 hover:bg-slate-900/90 hover:border-white/15'
                          }`}
                        >
                          ...
                          <h4 className={`font-semibold text-xs ${isSelected ? 'text-indigo-200 font-bold' : 'text-white'}`}>
                            {module.title}
                          </h4>
                    ...
                    <div className="col-span-full py-8 text-center text-slate-400 text-xs font-mono bg-slate-900/40 rounded-xl border border-white/5">
