...
import { hashId, enqueueIdempotent, withJobHeaders, pollJobResponse } from '@/lib/api/queueHttp';
...
type LinkPreview = { url: string; title?: string; faviconUrl?: string; meta?: string };
type AskPayload = { prompt: string; context?: string; role?: Role; mission?: string };
type CseItem = { title?: string; link?: string; snippet?: string };
type CseResponse = { items?: CseItem[] };

// -------- link utils (unchanged) ----------
...
function splitByCodeRegions(raw: string): Array<{ code: boolean; raw: string }> {
  ...
  while ((m = fenced.exec(raw))) { if (m.index > last) splitInline(raw.slice(last, m.index), out); out.push({ code: true, raw: m[0] }); last = fenced.lastIndex; }
  if (last < raw.length) splitInline(raw.slice(last), out);
  return out;
}
function splitInline(segment: string, out: Array<{ code: boolean; raw: string }>) {
  const inline = /`[^`\n]*`/g;
  let iLast = 0; let im: RegExpExecArray | null;
  while ((im = inline.exec(segment))) { if (im.index > iLast) out.push({ code: false, raw: segment.slice(iLast, im.index) }); out.push({ code: true, raw: im[0] }); iLast = inline.lastIndex; }
...
function extractLinksFromText(text: string, cap = 8): LinkPreview[] {
  ...
  while ((mdMatch = MD_RE.exec(text)) !== null) {
    ...
    if (!seen.has(u)) { seen.add(u); links.push(u); if (links.length >= cap) break; }
  }
  if (links.length < cap) {
  ...
  return links.map((url) => ({ url, title: safeHost(url), faviconUrl: `https://icons.duckduckgo.com/ip3/${safeHostname(url)}.ico` }));
}
function stripTracking(rawUrl: string): string {
  try { const u = new URL(rawUrl); ['utm_source','utm_medium','utm_campaign','utm_term','utm_content','gclid','fbclid'].forEach(p => u.searchParams.delete(p)); return u.toString(); }
  catch { return rawUrl; }
}
function safeHost(url: string): string { try { return new URL(url).hostname.replace(/^www\./, ''); } catch { return url; } }
function safeHostname(url: string): string { try { return new URL(url).hostname; } catch { return ''; } }
async function tryGoogleCse(query: string): Promise<LinkPreview[]> {
  try {
    ...
    url.searchParams.set('num', '3'); url.searchParams.set('q', query);
    const res = await fetch(url.toString(), { cache: 'no-store' });
    if (!res.ok) throw new Error(`Google CSE ${res.status}`);
    const data = (await res.json()) as CseResponse;
    return (data.items ?? []).filter((i): i is Required<CseItem> => !!i.link).map((i) => ({
      url: stripTracking(i.link!), title: i.title || safeHost(i.link!), meta: i.snippet,
...
function dedupeLinks(a: LinkPreview[], b: LinkPreview[]): LinkPreview[] {
  const out: LinkPreview[] = []; const seen = new Set<string>();
  for (const list of [a,b]) for (const l of list) { const key = stripTracking(l.url); if (seen.has(key)) continue; seen.add(key); out.push({ ...l, url: key }); }
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    // Use the interactive queue (by name) from the new provider
    const interactiveQueue = await getQueue(INTERACTIVE_QUEUE_NAME);

    // Keep job name aligned with type (worker maps by data.type anyway)
    ...
    const res = okJson({ error: 'Failed to enqueue ask job.', details: msg }, 500);
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  if (statsOnly || list) {
    ...
    if (statsOnly) {
      ...
      return NextResponse.json({ queues: { [INTERACTIVE_QUEUE_NAME]: iStats, [BACKGROUND_QUEUE_NAME]: bStats } });
    }
    ...
    const jobToJSON = async (j: Job<LlmJobData, LlmJobResult, string>) => ({ id: j.id, name: j.name, state: await j.getState() });
    const [iList, bList] = await Promise.all([Promise.all(iJobs.map(jobToJSON)), Promise.all(bJobs.map(jobToJSON))]);
    return NextResponse.json({ [INTERACTIVE_QUEUE_NAME]: iList, [BACKGROUND_QUEUE_NAME]: bList });
  }
  ...
  // Try to enrich finished 'ask' answers with linkification + CSE
  try {
    ...
    if (payload?.state === 'completed' && payload.result?.answer) {
      ...
      const textLinks = Array.isArray(existing) && existing.length > 0 ? existing : extractLinksFromText(answerWithLinks);
      // We don’t have original prompt here; best-effort: try to derive a short query from the answer’s first sentence.
