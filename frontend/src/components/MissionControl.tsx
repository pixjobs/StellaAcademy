...
function MissionControlInternal({
  ...
  }, [hasImages, initialMessage, role, buildContext, sendMessage]); // Dependency fix
  ...
  }, [hasImages, isChatLoading, images, selImageIndex, reset, mission, context, role, sendMessage]);

  const handleCaptureMessage = useCallback((message: Message) => {
    if (!message.text) return;
    const links = Array.from(message.text.matchAll(urlRegex), m => m[0]);
    const noteData = links.length > 0
      ? { type: 'reference' as const, title: 'Captured Link', url: links[0], body: message.text.replace(links[0], '').trim() }
      : { type: 'concept' as const, title: 'Captured Note', body: message.text };
    addNote(noteData);
  }, [addNote]);
  ...
  return (
    <TooltipProvider>
      <div ref={rootRef} className={clsx("flex flex-col gap-5 items-start min-h-0", hasImages && "xl:grid xl:grid-cols-[1.05fr_1.95fr]")}>
...
type TabButtonProps = { icon: ComponentType<{ className?: string }>; label: string; isActive: boolean; onClick: () => void };
const TabButton = memo(function TabButton({ icon: Icon, label, isActive, onClick }: TabButtonProps) {
  return (
    <Button onClick={onClick} variant="ghost" size="sm" className={clsx('flex-1 justify-center transition-colors duration-200', isActive ? 'bg-sky-900/30 text-white' : 'text-slate-400 hover:text-white')}>
...
const VisualPanel = memo(function VisualPanel({ currentImage, imageIndex, imageCount, onPrev, onNext, onQuickAction, onSaveImage }: VisualPanelProps) {
  ...
  return (
    <div className="w-full h-full flex flex-col gap-3 sticky top-4">
      <div ref={imageRef} className="relative w-full aspect-video rounded-xl overflow-hidden border border-white/10 bg-black/50 group">
        <Image key={currentImage.href} src={currentImage.href} alt={title} fill className="object-contain" sizes="(max-width: 1279px) 90vw, 33vw" />
        <div className="absolute bottom-0 left-0 right-0 px-3 py-2 text-xs bg-gradient-to-t from-black/80 to-transparent text-slate-200">{title} • #{imageIndex + 1}/{imageCount}</div>
        {imageCount > 1 && (
          <>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Tooltip><TooltipTrigger asChild><Button onClick={onSaveImage} size="sm"><Bookmark className="w-4 h-4 mr-1.5" />Save Image</Button></TooltipTrigger><TooltipContent>Save to Mission Notebook</TooltipContent></Tooltip>
      </div>
    </div>
  );
});

type ChatPanelProps = { messages: Message[]; isLoading: boolean; onSend: (text: string) => void; onStop: () => void; onCaptureMessage: (message: Message) => void; inputValue: string; setInputValue: Dispatch<SetStateAction<string>>; };
const ChatPanel = memo(function ChatPanel({ messages, isLoading, onSend, onStop, onCaptureMessage, inputValue, setInputValue }: ChatPanelProps) {
  return (
    <div className="chat-interface flex flex-col rounded-xl bg-white/5 border border-white/10 backdrop-blur-lg shadow-lg p-2 md:p-3 min-h-0 h-full">
      <div className="flex-1 min-h-0 overflow-y-auto"><ChatDisplay messages={messages} onCapture={onCaptureMessage} /></div>
